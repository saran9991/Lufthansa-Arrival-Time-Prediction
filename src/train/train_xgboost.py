...
if __name__ == "__main__":
    ...
    X_train = pd.read_csv(PATH_TRAIN_DATA, parse_dates=["arrival_time", "timestamp"])[::1000].reset_index(drop=True)
    X_train['flight_id'] = X_train.arrival_time
    y_train = seconds_till_arrival(X_train)

    print('Reading Testing DataFrame...')
    X_test = pd.read_csv(PATH_TEST_DATA, parse_dates=["arrival_time", "timestamp"])[::100].reset_index(drop=True)
