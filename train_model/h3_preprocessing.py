def h3_preprocess(data, res):
    ...
    final['h3_hour'] = final['timestamp'].dt.hour
    print('dt.hour completed')
    grouped = final.groupby(['h3index', 'h3_hour'])
    print('Grouping according to hour and h3index completed')

    final['hexbin_daily_density'] = grouped['h3index'].transform('count')
    final['average_daily_speed'] = grouped['groundspeed'].transform('mean')
    final['average_daily_altitude'] = grouped['altitude'].transform('mean')
    final['average_eta'] = grouped['y'].transform('mean')
